...
class NotesApi {
  ...
  private getDatabaseContent = async (databaseId: string): Promise<Note[]> => {
    ...
    console.log('DBBBBBBBBBBBBBBBBBBBBBBB_____' + databaseId);
      ...
      return {
        ...
        title:
          'title' in page.properties.Name
            ...
            : '',
        description:
          'rich_text' in page.properties.excerpt
            ? page.properties.excerpt.rich_text[0].plain_text
            : '',
        slug:
          'formula' in page.properties.slug_auto
            ? page.properties.slug_auto.formula.string
            : '',
        //   isPublished:
        //     'checkbox' in page.properties.published ? page.properties.published.checkbox : false,
        publishedAt:
          'date' in page.properties.publishedAt
            ? new Date(page.properties.publishedAt.date.start).toISOString()
            : '',
        // lastUpdatedAt:
        //   'date' in page.properties.last_updated ? page.properties.last_updated.date.start : '',
        //   inProgress:
